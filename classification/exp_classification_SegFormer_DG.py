...
for size in sizes:
    ...
    selected_weights_path = f"{segformer.weights_folder}/best_weights.h5"
    print(f"weights_path: {selected_weights_path}")
    segformer.segformer.load_weights(selected_weights_path)
    print()

    for city in cities:
        os.makedirs(f"{segformer.prediction_folder}/{city}", exist_ok=True)
        ...
        make_prediction_on_city(segformer.segformer, segformer.image_processor, segformer.prediction_folder, city_test_split_paths, city, channels_first, normalize)
        print()
    ...
    for city in cities:
        print(f"city = {city}")
        print(f"prediction_folder: {segformer.prediction_folder}")
        split_path = all_test_dirs[city]
        print(f"city test_splits_dir: {split_path}")
        test_masks, preds, test_split_paths = evaluate_model_on_city(None, mode, segformer.prediction_folder, split_path, city, evaluation_metrics, data_loader, None, channels_first)
